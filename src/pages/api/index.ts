...
if (_) {
    try {
        if (Number.isInteger(Number(_))) {
            maxInputTokens = Object.entries(maxInputTokens).reduce(
                (acc, [k]) => {
...
export const post: APIRoute = async context => {
    try {
        const from = context.request.headers.get('from')
        if (from !== 'baijiayun') {
        ...
        if (
            tokens >
            (body.key ? defaultMaxInputTokens[model] : maxInputTokens[model])
        ) {
